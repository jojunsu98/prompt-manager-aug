...
공식 과제 범위에 맞춰 Python 3.10 이상과 표준 라이브러리만 사용합니다. 변수,
문자열, Boolean, 리스트, 딕셔너리, 조건문, 반복문, 함수, `input()`, `print()`를
직접 확인할 수 있도록 초보자 수준의 구조를 유지했습니다.
...
- `아침 이슈 브리핑`은 초기 즐겨찾기 상태로 시작합니다.
- 새로 추가한 프롬프트의 즐겨찾기 기본값은 `False`입니다.

| 기본 카테고리 | 실제 기본 프롬프트 |
|---|---|
| 텍스트 생성 | AI 기술 뉴스 3줄 요약 |
| 이미지 생성 | 제품 썸네일 이미지 생성 |
| 영상 생성 | 15초 제품 광고 영상 구성 |
| 페르소나 | 친절한 고객 상담가 페르소나 |
| 자동화 | 아침 이슈 브리핑, 개인 지출 자동 분류 |
| 기타 | 회의 준비 체크리스트 |
...
- `categories`: 카테고리 문자열을 순서대로 담은 리스트
- `prompts`: 프롬프트 딕셔너리를 담은 리스트
- 프롬프트 필드: `title`, `content`, `category`, `favorite`
- 입력 검증: `add_prompt()`와 번호 선택 함수의 `while` 반복문
- 카테고리 조회: `show_by_category()`의 조건 필터링
- 검색: `search_prompt()`의 제목·내용 부분 문자열 및 대소문자 무시 비교
- 즐겨찾기: `toggle_favorite()`가 실제 Boolean 값을 변경
- 실행 제어: `main()`의 조건문과 `if __name__ == "__main__"` 보호문
...
Python 표준 라이브러리 `unittest`와 `unittest.mock`으로 작성한 50개 테스트를
다음 명령으로 실행합니다.
...
- [평가 가이드](docs/evaluation-guide.md)
- [평가 시연 대본](docs/demo-script.md)
- [초보자 학습 노트](docs/learning-notes.md)
...
3. `3`번을 반복해 기본 카테고리 1~6의 실제 결과 확인
4. `7`번에서 초기 즐겨찾기 확인
5. `4`번에서 검색 성공·실패·빈 검색어 확인
6. `6`번에서 즐겨찾기 추가·해제 후 `7`번에 즉시 반영되는지 확인
7. `1`번에서 빈 제목·내용 재입력과 새 카테고리 직접 입력 확인
8. `2`, `3`, `5`번에서 추가 데이터 유지·새 카테고리·상세 보기 확인
9. 문자, 음수, `99` 메뉴의 오류 안내 확인
10. `8`번에서 입력 없는 6단계 평가 데모 확인
11. `0`으로 종료한 뒤 다시 실행하여 기본 상태 초기화 확인
...
문자, 음수, 범위 밖 번호 등 잘못된 메뉴 입력도 별도로 안전하게 처리합니다.
...
이전 AI 활용 미션에서 사용한 3개 프롬프트를 보존하고, 6개 기본 카테고리가
모두 실제 데이터를 갖도록 총 7개로 확장했습니다.
...
최종 개선 버전은 다음 환경에서 직접 실행하고 확인하였습니다.

```text
Python 3.13.15
git version 2.55.0.windows.3
Git 사용자 정보: 로컬 설정 완료 (README에는 개인정보 미기록)
...
| `run_evaluation_demo()` | 현재 데이터로 주요 기능 6단계를 자동 시연하고 상태 복원 |
| `show_menu()` | 메인 메뉴 출력 |
| `main()` | 메뉴를 반복하고 선택한 기능을 실행하며 0에서 종료 |
